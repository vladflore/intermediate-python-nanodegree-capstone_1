...
class NEODatabase:
    ...
    def __init__(self, neos: List[NearEarthObject], approaches: List[CloseApproach]):
        ...
        self._neo_name_to_neos = collections.defaultdict(list)
        for approach in self._approaches:
            neo = self._neos.get(approach._designation, None)
            approach.neo = neo
            if neo:
                self._neo_name_to_neos[neo.name].append(neo)
    ...
    def get_neo_by_name(self, name):
        ...
        """
        neos = self._neo_name_to_neos[name]
        if neos:
            return neos[0]
        return None
