...
class NearEarthObject:
    ...
    def __init__(self, **info):
        ...
        """
        pdes = info.get('designation', '').strip()
        if not pdes:
            raise ValueError('\'designation\' is a required attribute.')
        else:
            self.designation = pdes

        iau_name = info.get('name', '').strip()
        self.name = iau_name if iau_name else None
        ...
        pha = info.get('hazardous', 'N').strip()
        if pha == 'Y':
            self.hazardous = True
        else:
...
class CloseApproach:
    ...
    def __str__(self):
        """Return `str(self)`."""
        return f'At {self.time_str}, \'{self.neo.fullname if self.neo else "n/a"}\' approaches Earth at a distance of {self.distance:.2f} au and a velocity of {self.velocity:.2f} km/s.'
